...
unsigned int nModifierInterval = 10 * 60; // 10 hours for new modifer computation
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    if(nHeight == 1){
        // Set aside for geolocal distribution
        nSubsidy = 49000000 * COIN;
    }
    else if(nHeight > 1 && nHeight < 7250)
    {
        // 5 Day Grace Period
        nSubsidy = MIN_TX_FEE;
    }
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 30 / (365 * 33 + 8);
